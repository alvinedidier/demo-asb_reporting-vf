...
// Fonction generate refactorisée
exports.generate = async (req, res) => {
    ...
    try {
      ...
      const nombreDiffDay = Utilities.nbr_jours(campaignDates.start, campaignDates.end).day;
      const daysUntilStart = Math.ceil((campaignDates.start - dateNow) / (1000 * 60 * 60 * 24));
      const daysUntilEnd = Math.ceil((campaignDates.end - dateNow) / (1000 * 60 * 60 * 24));
  
      // Calcul de la durée de la campagne
      const campaignDuration = Utilities.nbr_jours(campaignDates.start, campaignDates.end).day;
      ...
      // Nombre de jours restants pour la diffusion
      const remainingDays = Math.ceil((campaignDates.end - dateNow) / (1000 * 60 * 60 * 24));
      if (remainingDays > 365) {
        logger.info('Plus de 365 jours aprés la diffusion, prise de contact avec la régie recommandée.');
      }
      if (remainingDays > 0) {
      ...
      // Nombre de jours avant le lancement de la campagne
      const daysBeforeStart = Math.ceil((campaignDates.start - dateNow) / (1000 * 60 * 60 * 24));
      ...
      if (reportingData && reportingData.reporting_end_date < dateNow) {
        logger.info(`Affichage des données en cache pour la campagne: ${campaign.campaign_id}`);
        return res.render('report/template.ejs', {
          reporting: reportingData,
          moment,
          utilities: Utilities,
          timestamp_startdate: campaignDates.start.getTime(),
          date_now: dateNow,
          nombre_diff_day: nombreDiffDay,
        });
      } else {
        Utilities.manageCache(cacheStorageID);
